  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (receiptsResponse.success) {
        ...
        const totalSales = todayReceipts.reduce((sum, receipt) => sum + (receipt.total || receipt.amount || 0), 0);
        const transactions = todayReceipts.length;
        const cardPayments = todayReceipts.filter(receipt =>
          receipt.payment_method === 'card' || receipt.paymentMethod === 'card'
        ).length;
        const cardPaymentPercentage = transactions > 0 ? Math.round((cardPayments / transactions) * 100) : 0;
        ...
        setMetrics({
          ...
          transactionValueChange: 2 // Placeholder - in real app, this would come from comparison with previous period
        });
      }
    } catch (error) {
      ...
      <div className='grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4'>
        <Card>
          ...
          <CardContent className='px-6 pb-6'>
            <div className='text-2xl font-bold'>ETB {metrics.totalSales.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        ...
        <Card>
          ...
          <CardContent className='px-6 pb-6'>
            <div className='text-2xl font-bold'>ETB {metrics.avgTransactionValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            <p className='text-xs text-muted-foreground'>+{metrics.transactionValueChange}% from last week</p>
          </CardContent>
        </Card>
      </div>
    </div>
