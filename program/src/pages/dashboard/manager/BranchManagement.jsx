...
export function BranchManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingBranch) {
        const response = await managerService.updateBranch(
          editingBranch.id || editingBranch._id,
          formData
        );
        if (response.success) {
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm('Are you sure you want to delete this branch? This action cannot be undone.')
    ) {
      try {
      ...
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4'>
        <div>
          ...
          <p className='text-sm text-muted-foreground mt-1'>
            Manage your pharmacy branches across Ethiopia.
          </p>
        </div>
