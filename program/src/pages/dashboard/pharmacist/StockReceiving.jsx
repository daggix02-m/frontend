...
export function StockReceiving() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: deliveries.filter((d) => d.status === 'received').length,
      icon: CheckCircle,
      color: 'text-green-600',
    },
    {
      title: 'Total Value',
      value: 'ETB ' + deliveries.reduce((acc, curr) => acc + (curr.value || 0), 0).toLocaleString(),
      icon: Package,
                    ...
                    <TableCell>
                      {delivery.status === 'pending' ? (
                        <Button
                          size='sm'
                          variant='outline'
                          onClick={() => handleReceiveDelivery(delivery.id)}
                        >
                          Receive
                        </Button>
                      ) : (
          ...
          <CardContent>
            <div className='space-y-4'>
              <div className='grid gap-4 grid-cols-1 md:grid-cols-2'>
                <div>
                  ...
                  <p className='text-sm text-muted-foreground'>
                    PO Number: {selectedDelivery.poNumber}
                  </p>
                </div>
                <div>
                  <p className='text-sm font-medium'>
                    Expected Date: {selectedDelivery.expectedDate}
                  </p>
                  <p className='text-sm text-muted-foreground'>
                    Total Items: {selectedDelivery.items}
                  </p>
                </div>
              </div>
              ...
              <div className='flex gap-2'>
                ...
                <Button variant='outline' onClick={() => toast.info('Printing report...')}>
