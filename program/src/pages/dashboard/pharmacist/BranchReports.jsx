...
export function BranchReports() {
  ...
  const stats = [
    {
      title: 'Daily Sales',
      value: reportData.stats?.dailySales || 'ETB 0',
      icon: DollarSign,
      trend: reportData.stats?.dailySalesTrend || '0%',
      color: 'text-green-600',
    },
    {
      title: 'Transactions',
      value: reportData.stats?.transactions || '0',
      icon: ShoppingCart,
      trend: reportData.stats?.transactionsTrend || '0%',
      color: 'text-blue-600',
    },
    {
      title: 'Prescriptions Filled',
      value: reportData.stats?.prescriptionsFilled || '0',
      icon: FileText,
      trend: reportData.stats?.prescriptionsTrend || '0%',
      color: 'text-purple-600',
    },
    {
      title: 'Products Sold',
      value: reportData.stats?.productsSold || '0',
      icon: Package,
      trend: reportData.stats?.productsTrend || '0%',
      color: 'text-orange-600',
    },
    {
      title: 'Customers Served',
      value: reportData.stats?.customersServed || '0',
      icon: Users,
      trend: reportData.stats?.customersTrend || '0%',
      color: 'text-cyan-600',
    },
    {
      title: 'Avg Transaction',
      value: reportData.stats?.avgTransaction || 'ETB 0',
      icon: TrendingUp,
      trend: reportData.stats?.avgTransactionTrend || '0%',
      color: 'text-pink-600',
    },
  ];
          ...
          <div className='grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4'>
            ...
            <div className='text-center p-4 border rounded-lg'>
              <p className='text-2xl font-bold text-orange-600'>
                {reportData.inventorySummary?.lowStock || '0'}
              </p>
              <p className='text-sm text-muted-foreground mt-1'>Low Stock</p>
            </div>
            <div className='text-center p-4 border rounded-lg'>
              <p className='text-2xl font-bold text-red-600'>
                {reportData.inventorySummary?.expiringSoon || '0'}
              </p>
              <p className='text-sm text-muted-foreground mt-1'>Expiring Soon</p>
            </div>
            <div className='text-center p-4 border rounded-lg'>
              <p className='text-2xl font-bold'>
                {reportData.inventorySummary?.inventoryValue || 'ETB 0'}
              </p>
      ...
      <Card>
        ...
        <CardContent>
          <div className='grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3'>
            <div className='text-center p-4 border rounded-lg'>
              <p className='text-2xl font-bold'>
                {reportData.prescriptionStats?.filledToday || '0'}
              </p>
              <p className='text-sm text-muted-foreground mt-1'>Filled Today</p>
            </div>
            <div className='text-center p-4 border rounded-lg'>
              <p className='text-2xl font-bold'>
                {reportData.prescriptionStats?.pendingValidation || '0'}
              </p>
              <p className='text-sm text-muted-foreground mt-1'>Pending Validation</p>
            </div>
            <div className='text-center p-4 border rounded-lg'>
              <p className='text-2xl font-bold'>
                {reportData.prescriptionStats?.flaggedForReview || '0'}
              </p>
